...
class ArrayListSorter
{
    ...
    public function asc(callable $selector, bool $naturalSorting = FALSE, bool $caseSensitive = FALSE): ArrayListSorter
    {
        ...
        $this->comparators[] = [$selector, $flags];
     ...
     * @return int
     */
    private static function orderByCallback($item1, $item2, array $callbacks): int
    {
        ...
        if (is_string($value1) && is_string($value2)) {
            ...
            return ($result == 0) ?
                self::orderByCallback($item1, $item2, $callbacks) :
                $result;
        }

        if (($value1 instanceof \DateTime) && ($value2 instanceof \DateTime)) {
            if ($value1 == $value2)
                return self::orderByCallback($item1, $item2, $callbacks);
    ...
    /**
     * Execute sorting and return the list.
     *
     * @return ArrayList
     */
    public function process(): ArrayList
    {
        $callbacks = $this->comparators;

        usort($this->source,
            function ($item1, $item2) use ($callbacks) {
                return $this->orderByCallback($item1, $item2, $callbacks);
            });
